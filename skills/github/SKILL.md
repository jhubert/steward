...
- When using `--json`, field names are plural where applicable: `assignees` (not `assignee`), `labels`, `reviewers`. If unsure of a field name, run the command without `--json` first, or pass `--help` to list valid fields. Note there is no `merged` field on `pr view`/`pr list` — use `state` (value `"MERGED"`) or `mergedAt` instead.
- For `--jq` expressions that use jq string interpolation (e.g. `"\(.field)"`), wrap the whole `--jq` expression in single quotes so the double quotes survive word-splitting: `--jq '.items[] | "\(.id) \(.name)"'` — otherwise the inner double quotes are stripped as argument-splitting and jq sees a bare `\(` token, which is a syntax error.
